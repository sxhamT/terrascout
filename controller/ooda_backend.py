...
class OODABackend(Backend):
    ...
    def _phase_descend_scan(self, s, dt):
        ...
        # Zone commit: normal (min scans met AND drone is near-hover) OR forced at floor.
        # Speed gate: don't commit while the drone still has >0.8 m/s total speed —
        # APPROACH PID can't absorb the momentum and the drone dives past the 8m target.
    ...
    def _phase_approach(self, s, dt):
        ...
        Transitions to LAND once lateral error < 0.5 m (no altitude band needed —
        drone is already at 8m AGL by design when it reaches that position).
        """
